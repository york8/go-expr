...
func (v *Checker) builtinNode(node *ast.BuiltinNode) Nature {
	...
	case "each":
		collection := v.visit(node.Arguments[0])
		collection = collection.Deref(&v.config.NtCache)
		if !collection.IsArray() && !collection.IsMap() && !collection.IsStruct() && !collection.IsUnknown(&v.config.NtCache) {
			return v.error(node.Arguments[0], "builtin %v takes only array, map or struct (got %v)", node.Name, collection.String())
		}
		...
		v.begin(collection, varScope{"index", v.config.NtCache.FromType(intType)}, varScope{"key", v.config.NtCache.FromType(stringType)}, varScope{"acc", Nature{}}, varScope{"#each", Nature{}})
		predicate := v.visit(node.Arguments[1])
		v.end()
...
func (v *Checker) begin(collectionNature Nature, vars ...varScope) {
	scope := predicateScope{collection: collectionNature, vars: vars}
	for _, v := range vars {
		switch v.name {
		case "#each":
			scope.isEach = true
